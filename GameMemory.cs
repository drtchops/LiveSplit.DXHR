	...
	class GameMemory
	{
		...
		Process GetGameProcess()
		{
			...
			else  // DXHRDC
			{
				...
				if (_settings.DC_Option_DisableFocusLossBehaviour)
				{
					game.Suspend();
					try
					{
						...
						{
							0x74, 0x09, //JE
							0x83, 0x3D, 0x54, 0x02, 0xCC, 0x00, 0x00, //cmp dword ptr[DXHRDC.exe+A50254]
							0x75, 0x05, //jne [DXHRDC.exe+A5242]
							0xB0, 0x01, //<------- mov al, 01 !!
							0x5E, //pop esi
							0x59, //pop ecx
							0xC3  //ret
						...
						byte[] signature2 =
						{
							0x80, 0x78, 0x14, 0x00, //cmp byte ptr [eax+14],00
							0x74, 0x28, //je DXHRDC.exe+A52D8
							0xB0, 0x01, //<-------mov al,01
							0x5E, //pop esi
							0x59  //pop ecx
						};
						...
						if (game.ReadBytes(game.MainModule.BaseAddress + 0xA5232, signature1.Length, out var bytes1) && game.ReadBytes(game.MainModule.BaseAddress + 0xA52AD, signature2.Length, out var bytes2))
						{
							var eq1 = Enumerable.SequenceEqual(bytes1, signature1);
							var eq2 = Enumerable.SequenceEqual(bytes2, signature2);

							if (eq1 && eq2)
							{
								bool success = true;
								success &= game.WriteBytes(game.MainModule.BaseAddress + 0xA523E, new byte[] { 0x0 }); //patch mov al, 01 to mov al, 00
								success &= game.WriteBytes(game.MainModule.BaseAddress + 0xA52B4, new byte[] { 0x0 }); //patch mov al, 01 to mov al, 00
								if (!success)
									Debug.WriteLine($"[NoLoads] Failed to patch memory, maybe LiveSplit need to be run as elevated process?");
							}
							else
								Debug.WriteLine($"[NoLoads] Can not patch - might be that memory was already patched?!");
						}
					}
					catch (Exception ex)
					{
						Debug.WriteLine($"[NoLoads] There was an error trying to patch on focus behaviour: {ex}");
					}
					game.Resume();
				}
			}
